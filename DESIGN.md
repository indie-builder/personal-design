...
AI Coding 词典为用户确认的局部例外：保留全屏参考图谱，并将统一标题回退导航叠放在画布左上角、放射动效、节点标签及收起的圆形搜索入口，点击展开搜索。背景与前景色跟随本项目中性主题，字体使用 Albert Sans；七个章节以低饱和浅色节点区分，节点放大约 15%，径向碰撞修正保留原投影角度。详情采用 Impeccable 阅读面，当前详情要求取代此前保留原版详情比例与按钮的约束；图谱不重建。不恢复配色开关和音效。
...
| AI 问答 | 所有视口优先沿用移动视觉与交互；桌面居中480px窄框，宽屏时高calc(100dvh - 32px)、上下16px留白，外层返回标题独立悬浮不占据手机框布局；不足800px时页头预留72px避免重叠，639px及以下移除外框；消息区独立滚动，输入区在最新消息区域显示，浏览历史时带动效隐藏 |
...
**表单提交展示：** 用户消息仅呈现简短操作描述，提交数据默认折叠于“查看已提交内容”，展开后只读，沿用正文13px／标签12px与44px展开目标。结构化状态独立保存并提供给模型，重新生成、追问与摘要输入保持该上下文；不得把字段清单平铺为用户正文。

**历史回答只读：** 只有消息列表最后一条assistant回答且当前没有请求进行时允许编辑和执行生成操作。发送新问题后，先前回答立即锁定；等待或失败也不重新解锁旧回答。文本、选择、日期、滑块、单／多选、开关及编辑表格字段不可修改；生成Button／IconButton与操作卡片不能触发动作。锁定在编辑组件边界实施，onAction和onStateUpdate另有回传保护，不把历史回答伪装成streaming，也不整块inert。Table分页／左右滚动、Tabs、Accordion、编辑条目的查看展开、图片查看及复制保持可用，原填写结果保留。
...
图谱右下角保留收起式搜索：44px圆形入口，展开为最大280px胶囊，搜索在左、清除在右，图标18px / 1.6px；普通输入14px，粗指针16px。使用项目中性主题、单层内描边；聚焦只把同一条边界加深为2px，不叠加外圈、投影或输入框边界。搜索与清除均保留44px操作区，键盘和减少动态效果即时展开。

标题与关闭操作常驻，正文独立滚动；以标题、章节、中英释义、内容定位、逐段完整中英对照、收起的关联术语建立阅读顺序。中文概述不能替代正文译文；中英文同屏，每段对应，原文与译文表格均保留语义。关联术语默认只占一行，展开后按章节提供紧凑原生链接。底部相邻导航显示完整名称并允许换行，首末边界禁用。关闭与 Esc 清除选择并将焦点返回搜索入口；键盘与减少动态效果即时完成。所有尺寸统一使用页头标题旁的返回操作，不另放图谱内返回按钮。移除分享、Copy Markdown 及旧详情组件，不仅隐藏控件。具体流程见 [详情阅读区契约](docs/design/execution/ai-coding-dictionary-detail.md)。
