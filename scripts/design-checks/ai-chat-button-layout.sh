...
for(const [width,theme]of [[320,'light'],[390,'dark'],[1440,'light']]){
 ...
 const groups=await p.evaluate(()=>[...document.querySelectorAll('.openui-buttons,[class*="mobileEditActions"]')].map(e=>{const r=e.getBoundingClientRect();return {width:r.width,columns:getComputedStyle(e).gridTemplateColumns.split(' ').length,primary:e.querySelectorAll('.openui-button-base-primary').length,buttons:[...e.querySelectorAll(':scope > button,:scope > [data-edit-boundary] > button')].map(b=>{const a=b.getBoundingClientRect();return{label:b.textContent,width:a.width,height:a.height,left:a.left-r.left,top:a.top-r.top,radius:getComputedStyle(b).borderRadius,font:getComputedStyle(b).fontSize};})};}));
